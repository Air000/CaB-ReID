...
class build_transformer_local(EvaluationMode, nn.Module):
    ...
    def _part_memory_features(self, token_features, global_feat, part_masks, online_image=None, part_view_label=None):
        patch_count = token_features.shape[1] - 1
        token_h = getattr(self.base.patch_embed, 'num_y', int(patch_count ** 0.5))
        token_w = getattr(self.base.patch_embed, 'num_x', max(1, patch_count // max(1, token_h)))
        return self.cabreid(
            token_features,
            global_feat,
            masks=part_masks,
    ...
    def forward(self, x, label=None, cam_label=None, view_label=None, mask=None, part_view_label=None, return_part_features=False, feature_mode=None):
        ...
        if self.part_memory_enabled:
            part_labels = view_label if part_view_label is None else part_view_label
            global_feat, part_dict = self._part_memory_features(
                b1_feat, global_feat, mask, online_image=x, part_view_label=part_labels
            )
        ...
        if self.training:
            ...
            scores = [cls_score, cls_score_1, cls_score_2, cls_score_3, cls_score_4]
            feats = [global_feat, local_feat_1, local_feat_2, local_feat_3, local_feat_4]
            if return_part_features:
                return scores, feats, part_dict
            return scores, feats  # global feature for triplet loss
        else:
            if self.neck_feat == 'after':
                out = torch.cat([feat, local_feat_1_bn / 4, local_feat_2_bn / 4, local_feat_3_bn / 4, local_feat_4_bn / 4], dim=1)
            else:
                out = torch.cat([global_feat, local_feat_1 / 4, local_feat_2 / 4, local_feat_3 / 4, local_feat_4 / 4], dim=1)
            if return_part_features:
