...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--config_file", default="configs/trc31k/cabreid_v4_1_clipreid.yml", help="path to config file", type=str
    )
    ...
    train_loader_stage2, train_loader_stage1, part_cluster_loader, val_loader, num_query, num_classes, camera_num, view_num = make_dataloader(cfg)

    model = make_model(cfg, num_class=num_classes, camera_num=camera_num, view_num = view_num)
    if cfg.MODEL.PRETRAIN_CHOICE == 'self':
        if not cfg.MODEL.PRETRAIN_PATH:
            raise ValueError('MODEL.PRETRAIN_CHOICE self requires MODEL.PRETRAIN_PATH')
        logger.info('Loading baseline checkpoint for plug-in fine-tune: {}'.format(cfg.MODEL.PRETRAIN_PATH))
        model.load_param_finetune(cfg.MODEL.PRETRAIN_PATH)

    loss_func, center_criterion = make_loss(cfg, num_classes=num_classes)

    if cfg.MODEL.PRETRAIN_CHOICE == 'self':
        logger.info('Skipping CLIP-ReID stage1 prompt learning because a baseline checkpoint was loaded.')
    else:
